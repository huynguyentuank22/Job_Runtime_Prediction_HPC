...
class RNN(nn.Module):
    def __init__(self, input_size=6, hidden_size=64, num_layers=3, dropout=0.2):
        ...
        # Define RNN layer with batch_first=True for (batch, seq, feature) input format
        self.rnn = nn.RNN(input_size, hidden_size, num_layers, batch_first=True, dropout=dropout)
        # Linear layer to map RNN output to a single predicted value
    ...
    def train_model(self, model, train_loader, val_loader, epochs, lr, scaler):
        ...
        for epoch in range(epochs):
            ...
            for inputs, _, targets in train_loader:
                ...
                loss = criterion(outputs, targets)  # Compute Huber loss on inverse-transformed data
